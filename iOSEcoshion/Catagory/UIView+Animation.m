...
static const NSTimeInterval kMenuCellBouncingDuration = .2f;
static const CGFloat bigScale = 1.1;
static const CGFloat smallScale = .9;
...
- (void)shake
{
    ...
    anim.values = @[[NSValue valueWithCATransform3D:CATransform3DMakeRotation(-M_PI_4 / 4.f, 0, 0, 1)], [NSValue valueWithCATransform3D:CATransform3DMakeRotation(M_PI_4 / 4.f, 0, 0, 1)]];
    anim.autoreverses = YES ;
    anim.repeatCount = 10.0f ;
    anim.duration = 0.06f ;
...
- (void)playBouncing
{
    [UIView animateWithDuration:kMenuCellBouncingDuration animations:^{
        self.transform = CGAffineTransformScale(self.superview.transform, bigScale, bigScale);
    } completion:^(BOOL finished) {
        [UIView animateWithDuration:kMenuCellBouncingDuration animations:^{
            self.transform = CGAffineTransformScale(self.superview.transform, smallScale, smallScale);
        } completion:^(BOOL finished) {
            [UIView animateWithDuration:kMenuCellBouncingDuration animations:^{
